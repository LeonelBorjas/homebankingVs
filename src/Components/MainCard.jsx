...
const MainCard = () => {
    ...
    useEffect(() => {
        const fetchCards = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/clients/cards', {
                    headers: {
            ...
            <div className='flex flex-wrap justify-center items-center w-11/12 gap-4 mb-5'>
                {cards.map((card, index) => (
                    <div key={index} className='w-full my-2 flex flex-wrap justify-center items-center'>
                        <div className={`bg-white text-black rounded-lg p-4 w-80 ${flippedIndex === index ? 'flipped' : ''}`} onClick={() => handleCardClick(index)}>
